...
and design review and the search for an American round two. Every card rewritten the same day by Fable
for the page's goal, after Evan ruled that a lead magnet is not a strategy doc. A choose your own adventure case study:
...
|---|---|---|---|
| 1 | Repetition | Rico's Tacos, a twenty-two episode series, against a single Tide film, both P&G | 1,305,315 views across 22 episodes and 200,495 on episode 1, against 27,157 on the film, all read 14 September 2026 |
| 2 | Format and character, not just repeating | Mayhem, one character since 2010, against Check First, a different cast in each spot, both Allstate | MarketCast put Mayhem first among insurance mascots for likeability and for the most memorable ads; 31 percent of people in an Insurity survey picked him as the mascot they would most like to grab a beer with |
| 3 | Something people want to watch, about the audience not the brand | The Stratos jump against a Gives You Wiiings cartoon, both Red Bull | 49,048,257 views against 251,604 on the same channel, and more than 8 million concurrent livestreams of the jump, per YouTube |
| 4 | Own the show rather than sponsor someone else's | A YETI Presents film against YETI's name on a PBR bull riding award | 1,371,487 views on YETI's own channel against 13,732 on PBR's, and only one of those audiences belongs to YETI |
...
**Round 2's honest limit.** Both of its measures rank Mayhem against other insurers' mascots. We found no
published memorability score for Check First, and the card says so. Whether Check First's line is spoken in
the films is not verified, since no film is played in this workflow, so the wrong answer about repetition
speaks of the campaign name, which every upload title carries. Specsavers,
...
| File | What it is |
|---|---|
| `cards.json` | Fable's editorial decisions: every caption, category, title, spec row, stat, note, side name and why-answer. |
| `apply-decisions.js` | Copies the last json block of `review/card-decisions-fable-2026-09-14.md` into `cards.json`, word for word. |
...
Fable wrote the cards, and three other fresh Fable contexts reviewed them. The build session applied the
decisions word for word and adjudicated each finding against the source. The full release sequence in
`Writing/README.md` ran on 2026-09-14 for the reworked page. Everything is in `review/`:

| File | What it is |
|---|---|
| `card-decisions-fable-2026-09-14.md` | Fable's card decisions, with two revisions. The last json block is `cards.json`. |
| `independent-review-fable-cards-2026-09-14.md` | First review of the whole page. 7 clear findings, 9 judgment calls. |
| `adjudication-fable-cards-2026-09-14.md` | The ruling on each finding, with the facts checked. |
| `independent-review-final-fable-cards-2026-09-14.md` | Second fresh context. 3 blockers. |
| `adjudication-final-fable-cards-2026-09-14.md` | One blocker settled by evidence (the stills in `evidence/`), two sent to Fable. |
| `independent-review-changed-lines-final-2026-09-14.md` | Third fresh context. Both blockers resolved, verdict READY. |
| `review-record-fable-cards-2026-09-14.md` | All of the above embedded word for word, which the receipt fingerprints. |
| `review-receipt.json` | The receipt. Verified PASS. |

The files dated 2026-09-14 without "fable-cards" in the name cover the version before the rework and are kept
as history.
...
- **Resolved 14 September: the Rico's Tacos card.** Evan ruled that the page is a lead magnet, not a strategy
  doc, so the imported block, its FloQast cross-reference and its 24 August counts are gone. Fable rewrote the
  card from the playlist read on 14 September, and the gate now refuses any card taken from the block library.
- **Held for Evan and Tom: the eyebrow "A choose your own adventure case study".** A reviewer noted that the
  page does not branch.
- **Worth a 30-second watch:** whether "check Allstate first" is spoken in a Check First spot. If it is, the
  round 2 wrong answer can speak of a repeating line again, which needs a new review.
